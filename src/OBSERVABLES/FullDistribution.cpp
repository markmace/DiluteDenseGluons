...
namespace Observables{
    ...
    void DetermineFullDistribution(CVectorFields *OmS,CVectorFields *OmA){
        ...
        std::ofstream TwoDimDistribution; TwoDimDistribution.open(StringManipulation::StringCast(IO::OutputDirectory,"dNd2pID",MY_MPI_RNG_SEED,".txt").c_str());

//        DOUBLE MAX_LENGTHN=std::pow((Lattice::N[0]*Lattice::a[0])*(Lattice::N[1]*Lattice::a[1]),1.0/2.0);
//
//        INT NumberOfBinsN=8*INT(MAX_LENGTHN);
        
        DOUBLE MAXP=std::pow(1.0/(Lattice::a[0]*Lattice::a[1]),1.0/2.0);
        ...
        for(INT kXIndex=0;kXIndex<Lattice::N[0];kXIndex++){
            for(INT kYIndex=0;kYIndex<Lattice::N[1];kYIndex++){
                ...
                // DEFINE k^2 //
                DOUBLE kXLOC=IndexTok(kXIndex)*invfmtoGeV;
                DOUBLE kYLOC=IndexTok(kYIndex)*invfmtoGeV;

                DOUBLE kSqr=IndexTokSqr(kXIndex,kYIndex);
                DOUBLE kAbs=std::sqrt(kSqr)*invfmtoGeV;
        ...
        // SET OUTPUT FILE//
        std::string OutputFileN=StringManipulation::StringCast(IO::OutputDirectory,"Distribution","ID",MY_MPI_RNG_SEED,".txt");
